...
async function analyzeFeedCompliance(url) {
  ...
  if (violations > 0) {
    ...
    violationItems.slice(0, 3).forEach(v => {
      console.log(`   Item ${v.index}: ${v.width} cols → ${v.title.substring(0, 50)}...`);
    });
    if (violations > 3) {
...
async function demonstrateOneLiners() {
  ...
  try {
    ...
    console.log(`   RSS Audit: Title width ${auditResult.width} cols → ${auditResult.violation ? "VIOLATION" : "OK"}`);
...
async function main() {
  ...
  console.log("   15. Title audit: bun -e 'fetch(\"url\").then(r=>r.text()).then(t=>{const m=t.match(/<item[^>]*>[\\s\\S]*?<title[^>]*>([^<]+)<\\/title>/);const title=m?m[1].trim():\"No title\";const w=Bun.stringWidth(title);console.log(`RSS Audit: ${w} cols → ${w>89?\"VIOLATION\":\"OK\"}: ${title.substring(0,30)}...`)}'");
  console.log("   16. Audit logging: bun -e 'fetch(\"url\").then(r=>r.text()).then(async t=>{const m=t.match(/<item[^>]*>[\\s\\S]*?<title[^>]*>([^<]+)<\\/title>/);const title=m?m[1].trim():\"No title\";await Bun.write(\"audit.log\",`${new Date().toISOString()} | ${title}\\n`);console.log(\"Logged\")}')");
  console.log("   17. JSON export: bun -e 'fetch(\"url\").then(r=>r.text()).then(t=>{const items=t.match(/<item[^>]*>[\\s\\S]*?<\\/item>/g)||[];items.slice(0,3).forEach((i,idx)=>{const title=i.match(/<title[^>]*>([^<]+)<\\/title>/);const link=i.match(/<link[^>]*>([^<]+)<\\/link>/);console.log(`${idx+1}. ${JSON.stringify({title:title?title[1].trim():\"\",link:link?link[1].trim():\"\"})}`)}))'");
}
