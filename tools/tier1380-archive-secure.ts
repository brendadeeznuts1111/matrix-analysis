...
// ─── Argument Parser ───────────────────────────────────────────────────────
// @ts-ignore - Implicit any types for runtime compatibility
...
async function createArchive(sourcePath, outputPath, options = {}) {
  ...
  try {
    ...
    if (sourceStat.isFile()) {
      ...
      for await (const path of glob.scan(sourcePath)) {
        ...
        try {
          ...
          console.warn(`${GLYPH.warn} Skipping ${path}: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    } else {
    ...
    throw new Error(`Cannot access source path: ${sourcePath} - ${error instanceof Error ? error.message : String(error)}`);
  }
  ...
  if (!options.dry) {
    ...
    console.log(`${GLYPH.info} Files: ${fileCount}, Size: ${(totalSize / 1024 / 1024).toFixed(2)}MB`);
  ...
  if (!options.dry) {
    ...
    for (const [filePath, content] of Object.entries(files)) {
      const fileBuffer = content instanceof Uint8Array ? content : new Uint8Array(await content.arrayBuffer());
...
async function extractArchive(archivePath, outputPath, options = {}) {
  ...
  if (options.glob) {
    ...
    console.log(`${GLYPH.info} Extracted ${extractCount} entries matching pattern: ${options.glob}`);
  } else {
...
async function auditArchive(archivePath, options = {}) {
  ...
  for (const [path, file] of files) {
    ...
    if (options.verbose) {
      const fileHashBuffer = await crypto.subtle.digest('SHA-256', await file.arrayBuffer());
      const fileHashArray = Array.from(new Uint8Array(fileHashBuffer));
      const fileHash = fileHashArray.map(b => b.toString(16).padStart(2, '0')).join('');
  ...
  console.log(`${GLYPH.info} Total file size: ${(totalFileSize / 1024 / 1024).toFixed(2)}MB`);
...
async function generateArchiveSBOM(archivePath, files, options = {}) {
  ...
  for (const [filePath, content] of Object.entries(files)) {
    ...
    sbom.components.push({
      ...
      properties: [
        { name: "fileSize", value: fileBuffer.length.toString() }
      ]
    });
  }
...
function showArchiveAnalytics(tenant = 'default') {
  ...
  /** @type {any} */
  const total = AUDIT_DB.query("SELECT COUNT(*) as count FROM archive_operations WHERE tenant = ?").get(tenant);
  /** @type {any} */
  const creates = AUDIT_DB.query("SELECT COUNT(*) as count FROM archive_operations WHERE tenant = ? AND operation = 'create'").get(tenant);
  /** @type {any} */
  const extracts = AUDIT_DB.query("SELECT COUNT(*) as count FROM archive_operations WHERE tenant = ? AND operation = 'extract'").get(tenant);
  /** @type {any} */
  const totalFiles = AUDIT_DB.query("SELECT SUM(file_count) as total FROM archive_operations WHERE tenant = ? AND file_count IS NOT NULL").get(tenant);
  /** @type {any} */
  const totalSize = AUDIT_DB.query("SELECT SUM(total_size) as total FROM archive_operations WHERE tenant = ? AND total_size IS NOT NULL").get(tenant);
  ...
  console.log(`   Total size processed: ${totalSize?.total ? (totalSize.total / 1024 / 1024).toFixed(2) + 'MB' : '0MB'}`);

  // Recent operations
  /** @type {any[]} */
  const recent = AUDIT_DB.query("SELECT operation, archive_path, file_count, total_size, ts FROM archive_operations WHERE tenant = ? ORDER BY ts DESC LIMIT 5").all(tenant);
  if (recent.length > 0) {
    console.log(`\n   Recent operations:`);
    recent.forEach(op => {
      const size = op.total_size ? ` (${(op.total_size / 1024 / 1024).toFixed(1)}MB)` : '';
      console.log(`   ${op.operation.toUpperCase()}: ${op.archive_path?.split('/').pop()}${size} - ${new Date(op.ts).toLocaleTimeString()}`);
    });
  }
}
...
async function main() {
  ...
  if (!args.command || args.command === "--help") {
    ...
    console.log(`\nExamples:`);
    console.log(`  bun tools/tier1380-archive-secure.ts create ./src ./bundle.tar.gz --compress gzip --sbom`);
    console.log(`  bun tools/tier1380-archive-secure.ts extract ./bundle.tar.gz ./output --validate`);
    console.log(`  bun tools/tier1380-archive-secure.ts audit ./bundle.tar.gz --verbose`);
