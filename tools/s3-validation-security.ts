...
function testS3FileOperations() {
  ...
  for (const op of operations) {
    console.log(`\n${op.operation}:`);
    console.log(`  Valid params: pageSize=${op.validParams.pageSize || 'default'}, partSize=${op.validParams.partSize || 'default'}, retry=${op.validParams.retry || 'default'}`);
    console.log(`  Invalid params: pageSize=${op.invalidParams.pageSize || 'default'}, partSize=${op.invalidParams.partSize || 'default'}, retry=${op.invalidParams.retry || 'default'}`);
...
async function runS3ValidationTests(): Promise<void> {
  ...
  try {
    ...
    await Bun.write('./s3-validation-security-results.json', JSON.stringify(report, null, 2));
