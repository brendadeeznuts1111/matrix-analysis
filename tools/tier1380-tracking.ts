...
function showExecutionAnalytics() {
  ...
  const successRate = total && total.c > 0
...
async function scanCol89(file) {
  ...
  let total = 0, max = 0, violations = 0;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (w > COL_LIMIT) {
      violations++;
      const preview = Bun.escapeHTML(
        Bun.stripANSI(lines[i]).slice(0, 60)
      ) + "…";
      console.log(
        `${GLYPH.fail} Line ${i + 1}: ${w} cols → ${preview}`
      );
    }
  }
