...
async function checkETag(url, savedETag = null) {
  ...
  try {
    ...
    console.log(`   Status: ${isCached ? "✅ Not modified – use cache" : "🔄 Fresh fetch"}`);
...
async function benchmarkFetchAndParse(url, iterations = 3) {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    try {
      ...
      console.log(`     Fetch+Parse: ${totalTime}ms (Fetch: ${totalTime - parseTime}ms, Parse: ${parseTime}ms)`);
      console.log(`     Items: ${itemMatches.length}, Size: ${(text.length / 1024).toFixed(1)}KB`);
...
async function auditFeedSize(url) {
  ...
  try {
    ...
    itemMatches.forEach((itemText, index) => {
      ...
      if (index < 5) { // Store first 5 for detailed analysis
        itemDetails.push({
          index: index + 1,
          titleLength,
          descLength,
    ...
    if (itemDetails.length > 0) {
      ...
      itemDetails
        ...
        .forEach(item => {
          console.log(`   Item ${item.index}: ${item.totalLength} chars (Title: ${item.titleLength}, Desc: ${item.descLength})`);
        });
    }
...
async function demonstrateOneLiners() {
  ...
  try {
    const etagResult = await fetch("https://bun.com/rss.xml")
      .then(r => ({
        status: r.status,
...
async function simulateCacheWorkflow() {
  ...
  if (firstResult.etag) {
    ...
    // @ts-ignore - Type mismatch for runtime compatibility
    ...
    console.log(`   Bandwidth saved: ${secondResult.isCached ? `${(firstResult.content?.length || 0) / 1024}KB` : "0KB"}`);
    console.log(`   Cache efficiency: ${secondResult.isCached ? "✅ Optimal" : "⚠️  No caching benefit"}`);
  }
}
...
async function main() {
  ...
  console.log("   12. ETag check: bun -e 'fetch(\"url\").then(r=>console.log(r.status===304?\"Not modified\":\"Fresh fetch\",r.headers.get(\"etag\")))'");
  console.log("   13. Benchmark: bun -e 'const s=Date.now();fetch(\"url\").then(r=>r.text()).then(t=>console.log(\"Time:\",Date.now()-s,\"ms\"))'");
  console.log("   14. Size audit: bun -e 'fetch(\"url\").then(r=>r.text()).then(t=>{const items=(t.match(/<item/g)||[]).length;const descs=(t.match(/<description[^>]*>([^<]+)<\\/description>/gi)||[]).reduce((sum,d)=>sum+d.match(/<description[^>]*>([^<]+)<\\/description>/i)[1].length,0);console.log(`Items:${items},Desc chars:${descs},Avg:${Math.round(descs/items)}`)})'");
}
