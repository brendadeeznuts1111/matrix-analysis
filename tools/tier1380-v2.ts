 ...
 */
// @ts-ignore - Implicit any type for runtime compatibility
...
async function parseRSS(url) {
  try {
    ...
    // @ts-ignore - Bun.xml is experimental API not in TypeScript definitions
...
async function main() {
  ...
  db.prepare(
    "INSERT INTO scans (file, violations, max_width) VALUES (?, ?, ?)"
  ).run(target, col89.violations, col89.maxWidth);
