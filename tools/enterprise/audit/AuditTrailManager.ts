...
export class AuditTrailManager {
  ...
  async generateComplianceReport(
    ...
    const violations = this.db.query(`
    ...
    const recommendations = this.generateComplianceRecommendations(events, violations.count);
   ...
   */
  async applyRetentionPolicies(): Promise<{ processed: number; archived: number; deleted: number }> {
    ...
    for (const [eventType, policy] of this.retentionPolicies) {
      ...
      const eventsToProcess = this.db.query(`
  ...
  private evaluateRuleConditions(rule: ComplianceRule, event: AuditEvent): boolean {
    ...
    if (conditions.dataClassification && !conditions.dataClassification.includes(event.compliance.dataClassification)) {
      return false;
    }
    ...
    if (conditions.timeRestrictions) {
      ...
      if (conditions.timeRestrictions.startHour !== undefined && hour < conditions.timeRestrictions.startHour) {
        return false;
      }
      
      if (conditions.timeRestrictions.endHour !== undefined && hour > conditions.timeRestrictions.endHour) {
        return false;
      }
      
      if (conditions.timeRestrictions.daysOfWeek && !conditions.timeRestrictions.daysOfWeek.includes(dayOfWeek)) {
        return false;
      }
    }
  ...
  private async createViolation(eventId: string, ruleId: string, severity: string, description: string): Promise<void> {
  ...
  private analyzeRegulationCompliance(events: AuditEvent[]): ComplianceReport['regulations'] {
    const regulations: ComplianceReport['regulations'] = {};

    // Group events by regulation
    const eventsByRegulation = events.reduce((acc, event) => {
    ...
    for (const [regulation, regEvents] of Object.entries(eventsByRegulation)) {
      ...
      regulations[regulation] = {
        ...
        recommendations: compliant 
          ...
          : ['Investigate failure causes', 'Implement corrective actions', 'Enhance monitoring']
      };
    }
  ...
  private generateTrends(events: AuditEvent[]): ComplianceReport['trends'] {
    ...
    const eventVolume = Object.entries(eventsByDate).map(([date, count]) => ({ date, count }));

    // Analyze violation types
    const violationEvents = events.filter(e => e.outcome === 'failure');
    const violationTypes = violationEvents.reduce((acc, event) => {
    ...
    const accessPatterns = events.reduce((acc, event) => {
      if (event.userId) {
  ...
  private generateComplianceRecommendations(events: AuditEvent[], violations: number): string[] {
    const recommendations: string[] = [];

    if (violations > 0) {
      recommendations.push(`🚨 ${violations} compliance violations detected - immediate investigation required`);
    }

    const failureRate = events.filter(e => e.outcome === 'failure').length / events.length;
    if (failureRate > 0.1) {
      recommendations.push('⚠️ High failure rate detected - review operational procedures');
    }
    ...
    if (afterHoursEvents.length > events.length * 0.2) {
      recommendations.push('🌙 Significant after-hours activity - review access policies');
    }

    const restrictedDataAccess = events.filter(e => 
      e.compliance.dataClassification === 'restricted' && e.outcome === 'success'
    );

    if (restrictedDataAccess.length > 0) {
      recommendations.push('🔒 Restricted data access detected - ensure proper authorization and logging');
    }

    if (recommendations.length === 0) {
      recommendations.push('✅ Good compliance posture - continue monitoring and maintaining standards');
    }
