...
export class EnterpriseArchiveManager {
  ...
  constructor(tenantId: string = 'default', dbPath: string = './data/enterprise-archive.db') {
  ...
  async createSecureArchive(
    ...
    try {
      ...
      if (securityResult.riskLevel === 'critical') {
        throw new Error(`Critical security violations detected: ${securityResult.violations.join(', ')}`);
      }
      ...
      if (config.auditEnabled !== false) {
        await this.auditArchiveOperation(archiveId, 'create', metadata, metrics, securityResult);
      }

      console.log(`✅ Archive created: ${outputPath}`);
      console.log(`📊 Files: ${metadata.fileCount}, Size: ${(metadata.totalSize / 1024 / 1024).toFixed(2)}MB`);
      console.log(`⚡ Performance: ${metrics.creationTimeMs.toFixed(2)}ms, ${metrics.throughputMBps.toFixed(1)}MB/s`);
  ...
  async extractSecureArchive(
    ...
    try {
      ...
      if (config.auditEnabled !== false) {
        ...
        await this.auditArchiveOperation(archiveId, 'extract', metadata, metrics, securityResult);
      }
      ...
      console.error(`❌ Archive extraction failed: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  async analyzeArchive(archivePath: string): Promise<{
    ...
    try {
      ...
      // File analysis
      const fileAnalysis: { [key: string]: { size: number; type: string; risk: string } } = {};
      let totalSize = 0;
      ...
      console.log(`📊 Analysis complete: ${files.size} files, ${(totalSize / 1024 / 1024).toFixed(2)}MB`);
      console.log(`🔒 Security risk: ${securityResult.riskLevel}`);

      return { metadata, securityResult, fileAnalysis };

    } catch (error) {
      console.error(`❌ Archive analysis failed: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }

  // ─── Private Helper Methods ────────────────────────────────────────────────────
  private async collectFiles(sourcePath: string): Promise<{ files: { [key: string]: Uint8Array }; fileCount: number; totalSize: number }> {
    ...
    if (sourceStat.isFile()) {
      ...
      for await (const path of glob.scan(sourcePath)) {
        ...
        try {
          ...
          console.warn(`⚠️ Skipping ${path}: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    }
  ...
  private async validateSecurity(fileCollection: { files: { [key: string]: Uint8Array }; fileCount: number; totalSize: number }): Promise<SecurityValidationResult> {
  ...
  private calculatePerformanceMetrics(totalSize: number, timeMs: number): PerformanceMetrics {
  ...
  private assessFileRisk(path: string, fileType: string): string {
    if (path.includes('passwd') || path.includes('secret') || path.includes('key')) return 'high';
...
if (import.meta.main) {
  ...
  const tenantId = args.find(arg => arg.startsWith('--tenant='))?.split('=')[1] || 'default';
  ...
  try {
    switch (command) {
      case 'create':
        ...
        await archiveManager.createSecureArchive(sourcePath, { tenantId, auditEnabled: true });
        break;
      ...
      case 'extract':
        ...
        if (!archivePath || !extractPath) {
          console.error('❌ Usage: extract <archive-path> <extract-path> [--tenant=<id>]');
          process.exit(1);
        }
        await archiveManager.extractSecureArchive(archivePath, extractPath, { tenantId });
        break;
