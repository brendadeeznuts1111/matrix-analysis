...
import { EnterpriseSecurityValidator, securityValidator } from '../security/EnterpriseSecurityValidator';
...
export class EnterpriseArchiveCLI {
  ...
  private async handleCreate(args: string[], options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      this.showError(`Archive creation failed: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
  ...
  private async handleExtract(args: string[], options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      this.showError(`Archive extraction failed: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      archiveManager.close();
    }
  }
    ...
    try {
      ...
      if (options.verbose) {
        ...
        for (const [path, analysis] of Object.entries(result.fileAnalysis)) {
          const riskIcon = analysis.risk === 'high' ? '🚨' : analysis.risk === 'medium' ? '⚠️' : '✅';
          console.log(`  ${riskIcon} ${path}: ${analysis.size} bytes, ${analysis.type}, ${analysis.risk} risk`);
        }
      }
      ...
      this.showError(`Archive analysis failed: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      archiveManager.close();
    }
  }
  ...
  private async handleValidate(args: string[], options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      console.log(`📁 Total files: ${securityReport.blockedFiles.length + securityReport.allowedFiles.length}`);
      ...
      this.showError(`Security validation failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private async handleScan(args: string[], options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      this.showError(`Quick scan failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private async handleAudit(args: string[], options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      this.showError(`Audit report generation failed: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      archiveManager.close();
    }
  }
  ...
  private async handleMetrics(args: string[], options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      this.showError(`Metrics generation failed: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      archiveManager.close();
    }
  }
  ...
  private async handleBenchmark(args: string[], options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      this.showError(`Benchmark failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private async handleReport(args: string[], options: CLIOptions): Promise<void> {
    ...
    try {
      ...
      this.showError(`Report generation failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async run(argv: string[]): Promise<void> {
    ...
    try {
      ...
      this.showError(`Command failed: ${error instanceof Error ? error.message : String(error)}`);
      process.exit(1);
    }
  }
}
