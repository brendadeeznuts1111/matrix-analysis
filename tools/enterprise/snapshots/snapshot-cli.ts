...
async function handleCreate(): Promise<void> {
  ...
  if (!tenant) {
    ...
    console.error('   Options: --variant=<name> --pool-size=<n> --compression=<1-9> --include-config');
    return;
  }
  ...
  for (const opt of OPTIONS.slice(1)) {
    ...
    if (opt.startsWith('--compression=')) options.compressionLevel = parseInt(opt.split('=')[1]);
...
async function handleExtract(): Promise<void> {
  ...
  for (const opt of OPTIONS.slice(2)) {
    if (opt.startsWith('--max-size=')) options.maxExtractSize = parseInt(opt.split('=')[1]);
  }
...
async function handleCleanup(): Promise<void> {
  ...
  safeLog(`🗑️ Cleanup snapshots older than ${retentionDays} days (${dryRun ? 'DRY RUN' : 'EXECUTING'})`);
...
function showHelp(): void {
  ...
  console.log(`  bun snapshot-cli.ts create tenant-a --variant=production --compression=9`);
  console.log(`  bun snapshot-cli.ts list 20 --tenant=tenant-a`);
  console.log(`  bun snapshot-cli.ts extract ./snapshots/tenant-a-2026-01-31T12-34-56.tar.gz ./restore/`);
  console.log(`  bun snapshot-cli.ts verify ./snapshots/tenant-a-2026-01-31T12-34-56.tar.gz`);
