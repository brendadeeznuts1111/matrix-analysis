...
export class PerformanceAnalyzer {
  ...
  async generateReport(
    ...
    // Get benchmarks
    const benchmarks = this.db.query(`
    ...
    const peakMemoryUsage = metrics.length > 0
      ? Math.max(...metrics.map(m => m.memory_usage || 0))
      : 0;
    ...
    // Calculate trends
    const trends = {
      throughput: metrics.map(m => ({ timestamp: new Date(m.timestamp), value: m.throughput })),
      latency: metrics.map(m => ({ timestamp: new Date(m.timestamp), value: m.duration })),
      memory: metrics.map(m => ({ timestamp: new Date(m.timestamp), value: m.memory_usage || 0 }))
    };
   ...
   */
  getRealTimeMetrics(tenantId: string = 'default', limit: number = 100): PerformanceMetric[] {
    return this.metrics
   ...
   */
  analyzeTrends(tenantId: string = 'default', hours: number = 24): {
    ...
    if (throughputTrend === 'decreasing') {
      recommendations.push('⚠️ Throughput is decreasing - investigate performance bottlenecks');
    }
  ...
  private generateRecommendations(metrics: any[], benchmarks: any[]): string[] {
    ...
    if (metrics.length === 0) {
      recommendations.push('📊 No performance data available - start monitoring operations');
    ...
    if (avgThroughput < 10) {
      recommendations.push('🚀 Low throughput detected - consider optimizing compression settings or increasing resources');
    }

    if (avgLatency > 1000) {
      recommendations.push('⚡ High latency detected - investigate operation bottlenecks and optimization opportunities');
    }
    ...
    if (benchmarks.length > 0) {
      ...
      if (worstBenchmark.efficiency < 70) {
        recommendations.push(`📈 Low efficiency in ${worstBenchmark.operation} - consider optimization or refactoring`);
      }
    }
    ...
    if (mostFrequentOp) {
      recommendations.push(`🎯 Most frequent operation: ${mostFrequentOp[0]} (${mostFrequentOp[1]} times) - consider optimization priority`);
    }
