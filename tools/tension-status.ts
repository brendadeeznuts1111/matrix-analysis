...
// ── TENSION headers ─────────────────────────────────────────────────────────
try {
  const { stdout } = await $`rg -c '\\[TENSION\\-[A-Z]+\\-' --type=ts .`.quiet().nothrow();
  ...
  rows.push({ check: "TENSION headers", pattern: "\\[TENSION\\-[A-Z]+\\-", result: "SKIP", count: 0 });
}

// ── Anti-patterns: process.argv / process.env in src/ ───────────────────────
try {
  const { stdout } = await $`rg -c 'process\\.(argv|env)' --type=ts src/`.quiet().nothrow();
  const lines = stdout.toString().trim().split("\n").filter(Boolean);
  const total = lines.reduce((sum, line) => {
    const n = parseInt(line.split(":").pop() || "0", 10);
    return sum + n;
  ...
  rows.push({ check: "Anti-patterns (src/)", pattern: "process\\.(argv|env)", result: "CLEAN", count: 0 });
}

// ── Node.js imports in src/ ─────────────────────────────────────────────────
try {
  const { stdout } = await $`rg -c 'from "node:(fs|path|os|child_process)"' --type=ts src/`.quiet().nothrow();
  const lines = stdout.toString().trim().split("\n").filter(Boolean);
  const total = lines.reduce((sum, line) => {
    const n = parseInt(line.split(":").pop() || "0", 10);
    return sum + n;
  ...
  rows.push({ check: "Node imports (src/)", pattern: 'from "node:..."', result: "CLEAN", count: 0 });
}
...
try {
  ...
  rows.push({ check: "export default (src/)", pattern: "export default", result: "CLEAN", count: 0 });
}
...
try {
  ...
  rows.push({ check: "require() (src/)", pattern: "require\\(", result: "CLEAN", count: 0 });
}

// ── Seed / Snapshot Hash ────────────────────────────────────────────────────
const seed = Bun.randomUUIDv7();
const snapshotSrc = new Bun.CryptoHasher("sha256");
for (const path of ["src/cli.ts", "src/lib/profileLoader.ts", "src/lib/validators.ts", "src/lib/output.ts"]) {
...
try {
  ...
  rows.push({ check: "Commit Hash", pattern: "git rev-parse HEAD", result: "N/A", count: 0 });
}
...
try {
  ...
  const profProc = Bun.spawn(
    ...
    { stdout: "ignore", stderr: "ignore", cwd: profDir, env: { ...Bun.env, CLAUDECODE: "1" } },
  );
  await profProc.exited;

  // Find the generated .cpuprofile file
  const hotFns: { name: string; hits: number }[] = [];
  const { stdout: lsOut } = await $`ls ${profDir}/*.cpuprofile 2>/dev/null`.quiet().nothrow();
  ...
  if (profFile && await Bun.file(profFile).exists()) {
    try {
      ...
      for (const n of nodes) {
        if (n.hitCount > 0 && n.functionName && !["(idle)", "(program)", "(root)", "(garbage collector)"].includes(n.functionName)) {
  ...
  const top3 = hotFns.slice(0, 3).map((f) => `${f.name}(${f.hits})`).join(", ") || "N/A";
  rows.push({
  ...
  rows.push({ check: "CPU Hot Functions", pattern: "bun --cpu-prof", result: "SKIP", count: 0 });
}
...
const ENV_KNOBS = [
  { key: "TENSION_DECAY_RATE",    pattern: "TENSION_DECAY_RATE=0.92 bun propagate",       default: "0.95" },
  { key: "TENSION_DEBUG",         pattern: "TENSION_DEBUG=true bun runFullGraphPropagation", default: "false" },
  { key: "TENSION_SNAPSHOT_DIR",  pattern: "TENSION_SNAPSHOT_DIR=./snapshots bun propagate", default: "./tension-snapshots" },
] as const;
...
const found = rows.filter((r) => r.result === "FOUND" && r.check.includes("Anti")).length;
console.log(found === 0 ? "Status: ACTIVE" : `Status: ${found} anti-pattern(s) detected`);
