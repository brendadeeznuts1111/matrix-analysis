...
async function validateFeedXML(url) {
  ...
  try {
    ...
    if (!wellFormed) {
      ...
      return { valid: false, error: "XML structure issue", contentType, size: text.length };
    }
...
async function main() {
  ...
  console.log("   1. Feed structure: bun -e 'fetch(\"url\").then(r=>({ct:r.headers.get(\"content-type\"),t:r.text()})).then(async ({ct,t})=>({ct,size:(await t).length,hasRSS:(await t).includes(\"<rss\")})).then(console.log)'");
  console.log("   2. Content-Type: bun -e 'fetch(\"url\").then(r=>console.log(r.headers.get(\"content-type\")))'");
  console.log("   3. Basic metrics: bun -e 'fetch(\"url\").then(r=>r.text()).then(t=>console.log({size:t.length,items:(t.match(/<item/g)||[]).length}))'");
}
