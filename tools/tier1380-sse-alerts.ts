...
class AlertStreamManager {
  ...
  addClient(client: SSEClient) {
    ...
    console.log(`${GLYPHS.CONNECT} Client ${client.id.slice(0, 8)} connected on port ${this.config.port} (${this.clients.size} total)`);
  ...
  removeClient(id: string) {
    if (this.clients.has(id)) {
      ...
      console.log(`${GLYPHS.DISCONNECT} Client ${id.slice(0, 8)} disconnected (${this.clients.size} remaining)`);
    }
  }
  ...
  private async redisBroadcast(alert: ViolationAlert) {
    console.log(`${GLYPHS.BROADCAST} Redis: tier1380:violations:live (${alert.tenant}, port ${this.config.port})`);
  }
...
async function startSSEServer(config: Partial<SSEServerConfig> = {}) {
  ...
  console.log(`  ViolationAlert:   { id, timestamp, tenant, file, line, width, preview, severity, meta? }`);
  console.log(`  SSEClient:        { id, writer, tenant, connectedAt, filters?, sessionId? }`);
  ...
  console.log(`  Socket Options:   reusePort=${mergedConfig.socket?.reusePort}, keepAlive=${mergedConfig.socket?.keepAlive}`);
    ...
    async fetch(req) {
      ...
      if (pathname === "/mcp/alerts/stream") {
        ...
        const stream = new ReadableStream({
          ...
          cancel() { alertManager.removeClient(clientId); },
        });
      ...
      if (pathname === "/mcp/alerts/test" && req.method === "POST") {
        ...
        const alert: ViolationAlert = {
          ...
          severity: body.width > 120 ? "critical" : body.width > 100 ? "warning" : "info",
          meta: {
