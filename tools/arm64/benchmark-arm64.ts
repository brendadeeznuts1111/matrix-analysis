...
async function runBenchmark(
  ...
  const variance = samples.reduce((sum, time) => sum + Math.pow(time - avgTime, 2), 0) / samples.length;
...
async function benchmarkMultiCondition(): Promise<BenchmarkResult> {
  ...
  return runBenchmark(
    ...
    () => {
      ...
      index++;
      if (result && !(data.a && data.b && data.c && data.d)) throw new Error("Unexpected");
    },
    { iterations: 5000000 }
  );
}
...
function generateReport(results: BenchmarkResult[]): string {
  ...
  for (const result of results) {
    ...
    lines.push(`- **Operations/Second:** ${result.opsPerSecond.toLocaleString(undefined, { maximumFractionDigits: 0 })}`);
...
`);
  ...
  const reportPath = args[args.indexOf("--output") + 1] || args[args.indexOf("-o") + 1] || "arm64-benchmark-report.md";
