...
const TEST_PROFILES = [
  ...
  { name: "benchmark", description: "Performance-optimized", command: "bun run profile:benchmark" },
  { name: "test-performance", description: "Performance tests", command: "bun run profile:test-perf" },
  { name: "test-comprehensive", description: "Full comprehensive suite", command: "bun run profile:test-full" },
  { name: "integration", description: "Integration-focused", command: "bun run profile:integration" },
];
...
async function runTests(group: string | null, options: Record<string, any>): Promise<void> {
  ...
  const groupsToRun = group 
    ? (group === "all" ? TEST_GROUPS : TEST_GROUPS.filter(g => g.name === group))
    : TEST_GROUPS;
  ...
  console.log(`${fmt.info("Running tests")}: ${fmt.bold(groupsToRun.map(g => g.name).join(", "))}`);
  console.log();
  ...
  for (const testGroup of groupsToRun) {
    ...
    if (result.code !== 0) {
      ...
      console.log(`${fmt.fail("✗")} ${testGroup.name} failed with exit code ${result.code}`);
    } else {
  ...
  console.log(`─────────────────────────────────────`);
  console.log(`${fmt.bold("Summary")}: ${overallSuccess ? fmt.success("All passed") : fmt.fail("Some failed")} in ${totalTime}s`);
...
async function listGroups(): void {
  ...
  for (const group of TEST_GROUPS) {
    console.log(`${fmt.green(group.name).padEnd(12)} ${group.duration.padEnd(10)} ${group.description}`);
  }
  console.log();
...
async function useProfile(profileName: string): Promise<void> {
  ...
  if (profile.command) {
    ...
    console.log(`${fmt.info("Profile")}: ${fmt.bold(profileName)} - ${profile.description}`);
