...
async function createArchive(source, output, options = {}) {
  ...
  try {
    ...
    if (options.audit) {
      await logOperation('create', output, { fileCount, duration, tenant: options.tenant });
    }

  } catch (error) {
    console.error(`${GLYPH.fail} Error: ${error instanceof Error ? error.message : String(error)}`);
    throw error;
  }
}
...
async function extractArchive(archive, output, options = {}) {
  ...
  try {
    ...
    if (options.audit) {
      await logOperation('extract', archive, { entries: count, duration, tenant: options.tenant });
    }
...
async function inspectArchive(archive, options = {}) {
  ...
  try {
    ...
    console.log(`${GLYPH.info} Files: ${files.size}, Total content: ${(totalSize / 1024 / 1024).toFixed(2)}MB`);
    ...
    if (options.validate) {
      ...
      const suspicious = [...files.keys()].filter(p => 
...
async function main() {
  ...
  if (!args.command || args.command === "--help") {
    ...
    console.log(`\nExamples:`);
    console.log(`  bun tools/archive-tools.ts create ./src ./backup.tar.gz --compress gzip --audit`);
    console.log(`  bun tools/archive-tools.ts extract ./backup.tar.gz ./restore --validate`);
...
main().catch(e => {
  console.error(`${GLYPH.fail} Fatal error:`, e instanceof Error ? e.message : String(e));
  process.exit(1);
});
