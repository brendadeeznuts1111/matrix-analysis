...
async function checkCol89(filePath: string = Bun.main) {
  ...
  try {
    ...
    const violations = lines.filter((line) =>
      Bun.stringWidth(line, { countAnsiEscapeCodes: false }) > 89
    );
...
async function benchmarkCRC32() {
  ...
  return { throughput, duration, status: throughput > 1000 ? 'excellent' : throughput > 500 ? 'good' : 'moderate' };
}
...
async function getViolationCount() {
  ...
  try {
    ...
    const recent = db.query('SELECT COUNT(*) as c FROM violations WHERE ts > strftime("%s", "now", "-1 hour")').get() as { c: number };
    const today = db.query('SELECT COUNT(*) as c FROM violations WHERE ts > strftime("%s", "now", "start of day")').get() as { c: number };
    ...
    colorLog('yellow', `⚠️  Database not found or inaccessible: ${error?.message || error}`);
...
async function runSuite() {
  ...
  console.log(`🔍 Col-89: ${results.col89.violations === 0 ? '✅ PASS' : '❌ FAIL'} (${results.col89.violations} violations)`);
  console.log(`🚀 Hardware: ${results.hardware.throughput.toLocaleString()} MB/s (${results.hardware.status})`);
  console.log(`📊 Violations: ${results.violations.total} total, ${results.violations.recent} recent`);
  console.log(`🎨 CSS: ${results.css.status === 'success' ? `✅ ${results.css.saved}% saved` : '⚠️  ' + results.css.status}`);
  console.log(`📰 RSS: ${results.rss.status === 'success' ? `✅ ${results.rss.itemCount} items` : '❌ ' + results.rss.status}`);
  ...
  const healthColor = healthPercent >= 80 ? 'green' : healthPercent >= 60 ? 'yellow' : 'red';
...
export { checkCol89, benchmarkCRC32, getViolationCount, optimizeCSS, parseRSS, runSuite };
