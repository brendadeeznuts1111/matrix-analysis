...
export function validateToolCall(toolName: string, args: any): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
...
export function calculateComplianceScore(tenantId: string = "default"): ComplianceMetrics {
...
export function displayComplianceScore(metrics: ComplianceMetrics): void {
  ...
  console.log(`  Compliant Lines:    ${metrics.compliantLines.toLocaleString()} (${((metrics.compliantLines / metrics.totalLines) * 100).toFixed(1)}%)`);
  ...
  const scoreColor = metrics.score >= 90 ? COLORS.success : metrics.score >= 70 ? COLORS.warning : COLORS.error;
  console.log(`  Compliance Score:   ${scoreColor(metrics.score.toString())}/100`);
  console.log(`  Grade:              ${scoreColor(metrics.grade)}`);
  
  // Trend indicator
  const trend = metrics.score >= 95 ? "🟢 Excellent" : 
                metrics.score >= 80 ? "🟡 Good" : 
                metrics.score >= 60 ? "🟠 Needs Improvement" : "🔴 Critical";
...
export async function exportRegistryStats(format: "json" | "csv" = "json"): Promise<string> {
  const dbPath = "./data/tier1380.db";
  if (!existsSync(dbPath)) {
    return format === "json" ? "{}" : "";
  }
  ...
  const stats = {
    ...
    cache: db.query("SELECT COUNT(*) as count, SUM(size) as total FROM registry_cache").get() as any,
...
async function main() {
  ...
  switch (cmd) {
    case "validate": {
      ...
      if (!toolName) {
        console.error(`${GLYPHS.INVALID} Usage: bun run registry:validate <tool-name> [args-json]`);
        process.exit(1);
      }
      ...
      if (result.valid) {
        console.log(`${GLYPHS.VALID} ${COLORS.success("VALID")} (${duration.toFixed(2)}µs)`);
      } else {
        console.log(`${GLYPHS.INVALID} ${COLORS.error("INVALID")} (${duration.toFixed(2)}µs)`);
    ...
    case "list": {
      console.log(`${GLYPHS.SCHEMA} Registered Tools (${Object.keys(TOOL_SCHEMAS).length}):\n`);
