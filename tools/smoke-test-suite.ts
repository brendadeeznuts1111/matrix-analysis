...
class SmokeTestSuite {
  ...
  private async testHeaderCasePreservation(): Promise<void> {
    ...
    try {
      ...
      const hasCustomCase = "X-Custom-Case" in data.headers || "x-custom-case" in data.headers;

      console.log(`  Sent: ${YELLOW}X-Custom-Case: Value${RESET}`);
      console.log(`  Received: ${GREEN}${JSON.stringify(data.headers)}${RESET}`);
      console.log(`  Case preserved: ${hasCustomCase ? GREEN : RED}${hasCustomCase}${RESET}`);
  ...
  private async testProfileGeneration(): Promise<void> {
    ...
    try {
      ...
      const hasProfFlags = process.argv.includes("--cpu-prof") ||
                          process.argv.includes("--heap-prof");

      if (hasProfFlags) {
        console.log(`  Profile flags: ${GREEN}detected${RESET}`);
      } else {
        console.log(`  Profile flags: ${YELLOW}not in args${RESET} (pass --cpu-prof-md --heap-prof-md)`);
      }
  ...
  private printSummary(): void {
    ...
    console.log();
    console.log(`  ${GREEN}${passed} passed${RESET}, ${RED}${failed} failed${RESET}, ${this.results.length} total`);
