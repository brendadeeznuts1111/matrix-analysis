  ...
  try {
    switch (target) {
      ...
      case "bundle": {
        ...
        console.log(`${fmt.warn("Note")}: This would run bun build with the specified options`);
        console.log(`${fmt.dim("Would execute")}: bun build <entry> ${options.map(o => o.startsWith("--") ? o : `"${o}"`).join(" ")}`);
      ...
      case "optimize": {
        ...
        if (memoryOpt) console.log(`  ✓ Memory profiling${memoryOpt === "--memory:simple" ? " (simple)" : ""}`);
