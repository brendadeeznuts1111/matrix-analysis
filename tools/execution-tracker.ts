...
console.log("\n❌ Failure Analysis:");
const failures = db.query("SELECT COUNT(*) as c FROM executions WHERE exit_code != 0").get();
console.log(`Total Failures: ${failures.c}`);

const recentFailures = db.query("SELECT * FROM executions WHERE exit_code != 0 ORDER BY ts DESC LIMIT 3").all();
...
const slowExecutions = db.query("SELECT * FROM executions WHERE duration_ms > 100 ORDER BY duration_ms DESC LIMIT 3").all();
...
const successfulExecutions = db.query("SELECT COUNT(*) as c FROM executions WHERE exit_code = 0").get();
const successRate = totalExecutions.c > 0 ? (successfulExecutions.c / totalExecutions.c * 100).toFixed(1) : "0";
console.log(`Overall Success Rate: ${successRate}% (${successfulExecutions.c}/${totalExecutions.c})`);

// Example 6: Demonstrate tracking some sample executions
console.log("\n🎯 Tracking Sample Executions:");
const sampleCommands = [
  { cmd: "bun tools/tier1380-bunx.ts check", file: "/Users/nolarose/tools/tier1380-bunx.ts" },
  { cmd: "bun tools/tier1380-standalone check", file: "/Users/nolarose/tools/tier1380-standalone.ts" },
...
const updatedRecent = db.query("SELECT * FROM executions ORDER BY ts DESC LIMIT 8").all();
console.table(updatedRecent);

db.close();

console.log("\n💡 One-Liner Usage Examples:");
console.log("   • View recent: bun -e 'import{Database}from\"bun:sqlite\";const d=new Database(\"./data/executions.db\");console.table(d.query(\"SELECT * FROM executions ORDER BY ts DESC LIMIT 5\").all())'");
console.log("   • Check failures: bun -e 'import{Database}from\"bun:sqlite\";const d=new Database(\"./data/executions.db\");const f=d.query(\"SELECT COUNT(*) as c FROM executions WHERE exit_code != 0\").get();console.log(`Failures: ${f.c}`)'");
console.log("   • Success rate: bun -e 'import{Database}from\"bun:sqlite\";const d=new Database(\"./data/executions.db\");const s=d.query(\"SELECT COUNT(*) as c FROM executions WHERE exit_code = 0\").get();const t=d.query(\"SELECT COUNT(*) as c FROM executions\").get();console.log(`Success: ${(s.c/t.c*100).toFixed(1)}%`)'");
