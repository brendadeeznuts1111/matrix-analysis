...
function showAuditAnalytics(tenant = 'default') {
  ...
  /** @type {any} */
  const total = AUDIT_DB.query("SELECT COUNT(*) as count FROM executions WHERE tenant = ?").get(tenant);
  /** @type {any} */
  const successful = AUDIT_DB.query("SELECT COUNT(*) as count FROM executions WHERE tenant = ? AND exit = 0").get(tenant);
  /** @type {any} */
  const failed = AUDIT_DB.query("SELECT COUNT(*) as count FROM executions WHERE tenant = ? AND exit != 0").get(tenant);
  /** @type {any} */
  const avgThreatScore = AUDIT_DB.query("SELECT AVG(threat_score) as score FROM executions WHERE tenant = ? AND threat_score IS NOT NULL").get(tenant);
  ...
  console.log(`   Success rate: ${totalCount ? (successCount / totalCount * 100).toFixed(1) : '0'}%`);
  console.log(`   Failures: ${failCount}`);
  console.log(`   Avg threat score: ${avgScore ? (avgScore * 100).toFixed(1) + '%' : 'N/A'}`);

  // Recent executions
  /** @type {any[]} */
  const recent = AUDIT_DB.query("SELECT command, pkg, exit, threat_score, ts FROM executions WHERE tenant = ? ORDER BY ts DESC LIMIT 5").all(tenant);
  if (recent.length > 0) {
    console.log(`\n   Recent executions:`);
    recent.forEach(exec => {
      const status = exec.exit === 0 ? GLYPH.ok : GLYPH.fail;
      const threat = exec.threat_score ? ` (${(exec.threat_score * 100).toFixed(0)}%)` : '';
      console.log(`   ${status} ${exec.pkg || exec.command}${threat} - ${new Date(exec.ts).toLocaleTimeString()}`);
    });
  }
}
...
async function main() {
  ...
  if (!args.command) {
    ...
    console.log(`  bun tools/tier1380-exec-v2.ts --bun -p prettier@2.8.8 prettier --write src`);
  ...
  console.log(`${GLYPH.scan} Threat scan: ${threatCheck.safe ? GLYPH.ok : GLYPH.fail} ${threatCheck.score * 100}%`);
  ...
  if (!threatCheck.safe) {
    console.error(`${GLYPH.fail} Blocked: ${args.package || args.command} (threat score: ${threatCheck.score * 100}%)`);
    process.exit(1);
  }
  ...
  if (args.package) cmdParts.push("-p", `${args.package}${args.version ? "@" + args.version : ""}`);
  ...
  const safeCmd = Bun.escapeHTML(
    Bun.stringWidth(cmdLine) > 86 ? cmdLine.slice(0, 83).replace(/\S+$/, "").trim() + "…" : cmdLine
  );
