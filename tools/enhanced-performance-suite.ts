...
async function checkCol89Enhanced(filePath: string = Bun.main) {
  ...
  try {
    ...
    if (violations.length > 0) {
      ...
      console.log(`\n📊 Top 10 Worst Violations:`);
      worst.forEach(v => {
        const severity = v.width > 200 ? colors.bright_red : v.width > 150 ? colors.red : colors.yellow;
      ...
      console.log(`   • Lines over 150 chars: ${violations.filter(v => v.width > 150).length}`);
...
async function benchmarkHardwareEnhanced() {
  ...
  const tests = [
    { name: 'CRC32', size: 1 << 20, iterations: 100, func: (data: Uint8Array) => Bun.hash.crc32(data) },
    { name: 'String Width', size: 1000, iterations: 1000, func: () => Bun.stringWidth(''.padStart(1000, 'x')) }
  ];
  ...
  for (const test of tests) {
    ...
    const throughput = Math.round((test.iterations * test.size) / duration * 1000 / 1024 / 1024);

    results.push({ name: test.name, throughput, duration });

    const color = throughput > 1000 ? 'bright_green' : throughput > 500 ? 'green' : 'yellow';
    colorLog(color, `   ${test.name}: ${throughput.toLocaleString()} MB/s (${duration.toFixed(2)}ms)`);
  }

  // Overall score
  const avgThroughput = Math.round(results.reduce((sum, r) => sum + r.throughput, 0) / results.length);
  const score = avgThroughput > 1000 ? 'excellent' : avgThroughput > 500 ? 'good' : 'moderate';
...
async function analyzeViolationsEnhanced() {
  ...
  try {
    ...
    const queries = {
      ...
      lastHour: db.query('SELECT COUNT(*) as c FROM violations WHERE ts > ?').get(hourAgo) as { c: number },
      today: db.query('SELECT COUNT(*) as c FROM violations WHERE ts > ?').get(dayAgo) as { c: number },
      thisWeek: db.query('SELECT COUNT(*) as c FROM violations WHERE ts > ?').get(weekAgo) as { c: number }
    };

    // File breakdown
    const fileStats = db.query(`
    ...
    if (fileStats.length > 0) {
      ...
      fileStats.forEach(stat => {
        console.log(`   • ${stat.file}: ${stat.count} violations (max: ${stat.max_width} chars)`);
      });
    }

    // Trend analysis
    const hourlyRate = queries.lastHour.c;
    const trend = hourlyRate > 10 ? '🔴 High' : hourlyRate > 5 ? '🟡 Moderate' : '🟢 Low';
...
async function analyzeCSSEnhanced(cssFile: string = 'app.css') {
  ...
  try {
    ...
    if (lightningcss) {
      ...
      const savedPercentage = ((cssContent.length - result.code.length) / cssContent.length * 100).toFixed(1);
...
async function runEnhancedSuite() {
  ...
  if (results.violations.total !== undefined && typeof results.violations.total === 'number' && results.violations.total < 50) score++;
  else if (results.violations.total !== undefined && typeof results.violations.total === 'number' && results.violations.total < 100) score += 0.5;
  ...
  const healthColor = healthPercent >= 80 ? 'bright_green' : healthPercent >= 60 ? 'bright_yellow' : 'bright_red';

  // Enhanced summary
  colorLog('bright_magenta', '\n📋 Enhanced Summary Report');
  colorLog('bright_white', '='.repeat(60));

  console.log(`🔍 Col-89: ${results.col89.violations === 0 ? '✅ PASS' : results.col89.violations < 10 ? '⚠️  PARTIAL' : '❌ FAIL'} (${results.col89.violations} violations)`);
  if (results.col89.stats) {
    console.log(`   └─ Avg width: ${results.col89.stats.avgWidth} chars, Max: ${results.col89.stats.maxWidth} chars`);
  }

  console.log(`🚀 Hardware: ${results.hardware.avgThroughput.toLocaleString()} MB/s (${results.hardware.status})`);
  const hourlyRate = (results.violations as any)?.hourlyRate || (results.violations as any)?.lastHour?.c || 'N/A';
  const trend = (results.violations as any)?.trend || 'unknown';
  console.log(`📊 Violations: ${typeof results.violations.total === 'number' ? results.violations.total : 'N/A'} total, ${hourlyRate}/hour (${trend})`);
  console.log(`🎨 CSS: ${results.css.status === 'success' ? `✅ ${results.css.saved}% saved` : results.css.status === 'no_optimizer' ? '⚠️  No optimizer' : '❌ Error'}`);
  console.log(`📰 RSS: ${results.rss.status === 'success' ? `✅ ${results.rss.itemCount} items` : '❌ Error'}`);
...
export { checkCol89Enhanced, benchmarkHardwareEnhanced, analyzeViolationsEnhanced, analyzeCSSEnhanced, analyzeRSSEnhanced, runEnhancedSuite };
