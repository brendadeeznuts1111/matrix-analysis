...
const SKIP = ["node_modules", "dist", ".git", "coverage"];

const args = Bun.argv.slice(2);
const verbose = args.includes("--verbose") || args.includes("-v");
const jsonOut = args.includes("--json");
const showMetrics = args.includes("--metrics") || args.includes("-m");
const showPatterns = args.includes("--patterns") || args.includes("-p");
const showRegex = args.includes("--regex") || args.includes("-r");
const dirs = args.filter((a) => !a.startsWith("-"));
const scanDirs = dirs.length > 0 ? dirs : DIRS;
