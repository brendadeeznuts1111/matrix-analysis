...
export class ArchiveCompressionEngine {
  ...
  private initializeStrategies(): void {
    ...
    this.strategies.set('gzip', {
      ...
      estimateCompressionRatio: (data: Uint8Array) => {
        ...
        const text = new TextDecoder().decode(data.slice(0, Math.min(1024, data.length)));
    ...
    if (enableCaching && this.cache.has(cacheKey)) {
      ...
      return {
        ...
        metrics: {
          ...
          strategy,
          level: level || compressionStrategy.getOptimalLevel(originalSize, 'application/octet-stream')
        }
      };
    }

    // Perform compression
    const optimalLevel = level || compressionStrategy.getOptimalLevel(originalSize, 'application/octet-stream');
   ...
   */
  async decompress(data: Uint8Array, strategy: string = 'gzip'): Promise<{ decompressed: Uint8Array; metrics: Partial<CompressionMetrics> }> {
    const compressionStrategy = this.strategies.get(strategy);
    if (!compressionStrategy) {
      throw new Error(`Unknown compression strategy: ${strategy}`);
    }
  ...
  async benchmark(data: Uint8Array): Promise<CompressionBenchmark> {
    console.log(`🏁 Running compression benchmark on ${(data.length / 1024).toFixed(1)}KB data...`);
    ...
    for (const strategyName of strategyNames) {
      try {
        ...
        // Decompression
        const decompressResult = await this.decompress(compressResult.compressed, strategyName);
        ...
        console.log(`    ✅ ${strategyName}: ${(compressResult.metrics.compressionRatio * 100).toFixed(1)}% ratio, ${compressResult.metrics.compressionTime.toFixed(2)}ms`);

      } catch (error) {
        console.error(`    ❌ ${strategyName} failed: ${error instanceof Error ? error.message : String(error)}`);
      }
    }

    // Determine winner (best compression ratio with reasonable speed)
    const validMetrics = Object.entries(metrics).filter(([_, m]) => m.compressionRatio < 0.95); // Exclude useless compression
    const winner = validMetrics.reduce((best, [name, metrics]) => {
   ...
   */
  getOptimalStrategy(data: Uint8Array, contentType: string = 'application/octet-stream'): {
    strategy: string;
    level: number;
  ...
  private maintainCacheSize(): void {
    ...
    let totalSize = Array.from(this.cache.values()).reduce((sum, data) => sum + data.length, 0);
  ...
  private generateRecommendation(metrics: { [strategy: string]: CompressionMetrics }, winner?: string): string {
    if (!winner) {
  ...
  private generateReasoning(strategy: string, dataSize: number, contentType: string): string {
