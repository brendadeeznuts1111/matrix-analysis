...
async function validateFeedXML(url) {
  ...
  try {
    ...
    if (!isRSS && !isAtom) {
      ...
      return { valid: false, error: "Not RSS/Atom feed", contentType, size: text.length };
    }
    
    // Extract feed info
    const title = doc.querySelector("title")?.textContent?.trim() || "No title";
    const items = doc.querySelectorAll("item").length + doc.querySelectorAll("entry").length;
...
async function quickHealthChecks() {
  ...
  try {
    const contentType = await fetch("https://bun.com/rss.xml")
      .then(r => r.headers.get("content-type") ?? "missing");
...
async function main() {
  ...
  console.log("\n💡 One-Liner Examples:");
  console.log("   1. XML validity: bun -e 'await fetch(\"url\").then(r=>r.text()).then(t=>{try{new DOMParser().parseFromString(t,\"application/xml\");console.log(\"Valid\")}catch(e){console.log(\"Invalid:\",e.message)}})'");
  console.log("   2. Error detection: bun -e 'await fetch(\"url\").then(r=>r.text()).then(t=>{const d=new DOMParser().parseFromString(t,\"application/xml\");console.log(d.querySelector(\"parsererror\")?\"Error\":\"OK\")}'");
  console.log("   3. Content-Type: bun -e 'await fetch(\"url\").then(r=>console.log(r.headers.get(\"content-type\")))'");
}
