...
async function main() {
  ...
  console.log(
    `${colStatus} Col-89: ${col89.violations} violations ` + `(max ${col89.maxWidth}, avg ${col89.avgWidth})`,
  );
  ...
  if (css.status === "success") {
    console.log(`${GLYPH.pass} CSS: ${css.saved} reduction ` + `(${css.minified}/${css.original} bytes)`);
  } else {
