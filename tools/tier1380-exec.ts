...
async function verifyPackage(
  ...
  const auditHash = Bun.hash
    .wyhash(Buffer.from(fullName))
    .toString(16)
    .slice(0, 16);
...
function showStats(): void {
  ...
  const failures = (db.query("SELECT COUNT(*) as c FROM executions WHERE exit != 0").get() as any).c;
  const suspicious = (db.query("SELECT COUNT(*) as c FROM packages WHERE reputation = 'suspicious'").get() as any).c;
  ...
  console.log(`  Failures:         ${failures}${execCount > 0 ? ` (${((failures / execCount) * 100).toFixed(1)}%)` : ""}`);
...
async function main(): Promise<void> {
  ...
  const { safe, hash, cached, reputation } = await verifyPackage(
    command,
    flags.version
  );
  ...
  const { exitCode, durationMs } = await executeSecure(
    command,
    commandArgs,
    flags
  );
