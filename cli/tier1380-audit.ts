...
if (!Bun.semver.satisfies(Bun.version, MIN_BUN)) {
  console.error(
    `[TIER-1380] Bun ${Bun.version} < ${MIN_BUN} — upgrade`
  );
  process.exit(1);
}
...
function recordAudit(
  ...
  db.prepare(
    "INSERT INTO audits (command, duration_ms, result) " +
      "VALUES (?, ?, ?)"
...
async function checkCol89(filePath: string): Promise<number> {
  ...
  const stmt = db.prepare(
    "INSERT INTO violations (file, line, width, preview) " +
      "VALUES (?, ?, ?, ?)"
  );
...
async function optimizeCSS(inputPath: string): Promise<void> {
  ...
  if (!(await Bun.file(inputPath).exists())) {
    console.error(
      `${GLYPHS.AUDIT} File not found: ${inputPath}`
    );
    return;
  }
  ...
  try {
    ...
    console.error(
      `${GLYPHS.AUDIT} lightningcss not installed.` +
  ...
  const saved = (
    (1 - result.code.length / code.length) * 100
  ).toFixed(1);
  const durationMs = (Bun.nanoseconds() - start) / 1e6;
...
async function checkRSS(
  ...
  if (!res || !res.ok) {
    console.error(
      `${GLYPHS.AUDIT} Failed to fetch: ${feedUrl}`
    );
    return;
  }
  ...
  recordAudit(
    `rss ${feedUrl}`,
    durationMs,
    `${items.length} items, ${text.length}B`
  );

  console.log(
    `${GLYPHS.DEPENDENCY_COHERENCE}` +
      ` RSS Audit (${durationMs.toFixed(0)}ms):`
  );
  console.log(`  Feed:  ${feedUrl}`);
  console.log(
    `  Size:  ${text.length.toLocaleString()} bytes`
  );
...
function auditGlobals(): void {
  ...
  for (const key of keys) {
    ...
    if (/hash|sha|md[45]|crc|password|csrf|crypto/i.test(key)) {
      ...
      /sql|sqlite|s3|redis|json|toml|yaml/i.test(key)
    ) {
      grouped.data.push(entry);
    } else if (
      /serve|fetch|connect|listen|dns|spawn|udp/i.test(key)
    ) {
      grouped.network.push(entry);
    } else if (
      /inspect|color|string|strip|wrap|escape|sleep|deep|semver|which|peek/i.test(
        key
      )
    ) {
  ...
  const health = [
    {
      metric: "Bun version",
      value: Bun.version,
      status: Bun.semver.satisfies(Bun.version, MIN_BUN)
        ? "ok"
        : "upgrade",
    },
    {
...
function launchDashboard(): void {
  ...
  console.log(
    `${GLYPHS.STRUCTURAL_DRIFT} Dashboard:` +
