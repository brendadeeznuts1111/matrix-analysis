...
function testURLSearchParamsConfigurable() {
  ...
  const descriptor = Object.getOwnPropertyDescriptor(params.constructor.prototype, 'size');
...
async function runWebAPITests(): Promise<void> {
  ...
  try {
    ...
    const report = {
      ...
      webAPIFixes: {
        urlSearchParamsConfigurable: 'Fixed - size property now configurable per Web IDL',
        websocketDecompressionProtection: 'Fixed - 128MB limit prevents memory exhaustion',
    ...
    await Bun.write('./webapi-security-fixes-results.json', JSON.stringify(report, null, 2));
