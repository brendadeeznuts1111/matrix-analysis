  ...
  async listProfiles(): Promise<string[]> {
    try {
      ...
      const profileFiles = files.filter(file => 
        file.key?.startsWith('profiles/') && file.key?.endsWith('.md')
      );
      
      console.log(`📋 Found ${profileFiles.length} profile files in R2`);
      return profileFiles.map(f => f.key);
    } catch (error) {
...
class ProfileManager {
  ...
  async getLatestProfiles(): Promise<{ cpu: { key: string; content: string } | null; heap: { key: string; content: string } | null }> {
    ...
    try {
      // Find profile files using Bun's file system
      const cpuFiles = await Array.fromAsync(
        new Bun.Glob(`${this.profilesDir}/cpu-*.md`).scan()
      );
      const heapFiles = await Array.fromAsync(
        new Bun.Glob(`${this.profilesDir}/heap-*.md`).scan()
      );
      ...
      if (cpuFiles.length > 0) {
        const cpuFile = cpuFiles[0];
        const content = await Bun.file(cpuFile).text();
        const filename = cpuFile.split('/').pop() || '';
        const timestamp = filename.replace('cpu-', '').replace('.md', '');
        cpuProfile = { key: `profiles/cpu/${timestamp}-cpu-profile.md`, content };
      }
      
      if (heapFiles.length > 0) {
        const heapFile = heapFiles[0];
        const content = await Bun.file(heapFile).text();
        const filename = heapFile.split('/').pop() || '';
        const timestamp = filename.replace('heap-', '').replace('.md', '');
