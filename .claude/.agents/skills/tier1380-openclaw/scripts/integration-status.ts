...
async function checkGitHooks(projectPath: string) {
  ...
  try {
    hooks.commit = await Bun.file(`${projectPath}/.git/hooks/post-commit-topic`).exists();
...
async function getRepoInfo(projectPath: string) {
  try {
    const remote = await $`cd ${projectPath} && git remote get-url origin 2>/dev/null || echo "none"`.text();
    const branch = await $`cd ${projectPath} && git branch --show-current 2>/dev/null || echo "none"`.text();
    return {
      remote: remote.trim(),
      branch: branch.trim(),
    };
...
async function showDashboard() {
  ...
  console.log("╔══════════════════════════════════════════════════════════════════════════╗");
  console.log("║     🔗 TOPIC-PROJECT-CHANNEL INTEGRATION STATUS                          ║");
  ...
  for (const [id, topic] of Object.entries(topics)) {
    ...
    const color = priority === "high" ? COLORS.red : priority === "low" ? COLORS.gray : COLORS.green;
    console.log(`  ${icon} Topic ${id}: ${t.name.padEnd(15)} ${color}[${priority}]${COLORS.reset}`);
  }
  ...
  for (const [name, st] of Object.entries(superTopics)) {
    ...
    console.log(`  ${superTopic.icon || "📦"} ${name.padEnd(15)} → Topics: [${superTopic.topics?.join(", ") || "none"}]`);
  }
  ...
  for (const [name, proj] of Object.entries(projects)) {
    ...
    const hooks = hasRepo ? await checkGitHooks(project.path) : { commit: false, merge: false };
    const repoInfo = hasRepo ? await getRepoInfo(project.path) : { remote: "none", branch: "none" };
    
    const repoStatus = hasRepo ? `${COLORS.green}●${COLORS.reset}` : `${COLORS.red}○${COLORS.reset}`;
    const hookStatus = hooks.commit && hooks.merge 
      ? `${COLORS.green}✓ hooks${COLORS.reset}` 
      : `${COLORS.yellow}⚠ hooks${COLORS.reset}`;
    
    console.log(`  ${repoStatus} ${name.padEnd(22)} → Topic ${project.default_topic}  ${hookStatus}`);
    console.log(`    Path: ${project.path}`);
    if (hasRepo && repoInfo.remote !== "none") {
      console.log(`    Repo: ${COLORS.gray}${repoInfo.remote}${COLORS.reset} (${repoInfo.branch})`);
    }
  }
  ...
  const hookHealth = totalRepos > 0 ? Math.round((reposWithHooks / totalRepos) * 100) : 0;
  const healthColor = hookHealth >= 80 ? COLORS.green : hookHealth >= 50 ? COLORS.yellow : COLORS.red;
  
  console.log(`  Projects Configured:  ${totalProjects}`);
  console.log(`  Git Repositories:     ${totalRepos}`);
  console.log(`  Hooks Installed:      ${reposWithHooks}/${totalRepos} ${healthColor}(${hookHealth}%)${COLORS.reset}`);
  
  // Quick Actions
  console.log(`\n${COLORS.bold}⚡ Quick Actions${COLORS.reset}`);
  console.log("─".repeat(74));
  console.log(`  ${COLORS.cyan}kimi hooks install${COLORS.reset}        Install git hooks for all projects`);
  console.log(`  ${COLORS.cyan}kimi watch start${COLORS.reset}          Start file watcher`);
  console.log(`  ${COLORS.cyan}kimi project list${COLORS.reset}         List all projects`);
  console.log(`  ${COLORS.cyan}kimi topic list${COLORS.reset}           List all topics`);
  console.log(`  ${COLORS.cyan}kimi channel dashboard${COLORS.reset}    Open channel monitor`);
...
async function showMatrix() {
  ...
  for (const [name, proj] of Object.entries(projects)) {
    ...
    for (const id of Object.keys(topics)) {
      const mapped = project.topics?.[id] ? "●" : project.default_topic === parseInt(id) ? "○" : " ";
