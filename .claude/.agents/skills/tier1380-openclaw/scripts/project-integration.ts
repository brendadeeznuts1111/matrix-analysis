...
const CONFIG_PATH = join(homedir(), ".kimi", "skills", "tier1380-openclaw", "config", "project-topics.yaml");
...
class ProjectIntegration {
  ...
  listProjects(): void {
    ...
    for (const [name, project] of Object.entries(this.config.projects)) {
      ...
      console.log(`     ${COLORS.gray}Default Topic:${COLORS.reset} ${project.default_topic}`);
      console.log();
    }
  }
  ...
  listGroups(): void {
    ...
    for (const [key, group] of Object.entries(this.config.groups)) {
      ...
      console.log(`     ${COLORS.gray}Projects:${COLORS.reset} ${group.projects.join(", ")}`);
      console.log(`     ${COLORS.gray}Topics:${COLORS.reset} ${group.topics.join(", ")}`);
      console.log();
    }
  }
  ...
  routeForProject(projectName: string, message: string, fileType?: string): { topic: number; reason: string } | null {
    ...
    // Check routing rules
    for (const [pattern, topicId] of Object.entries(this.config.routing.commit_patterns)) {
  ...
  notify(projectName: string, event: string, data: Record<string, string>): void {
    ...
    if (!project.notifications[notificationKey] && !project.notifications[event]) {
      console.log(`${COLORS.gray}Notification disabled for ${event} in ${projectName}${COLORS.reset}`);
      return;
    }
  ...
  switch (command) {
    ...
    case "current":
      ...
      if (current) {
        ...
        console.log(`${COLORS.yellow}No project detected for current directory${COLORS.reset}`);
      }
      break;
    ...
    case "route":
      ...
      if (!projectName || !message) {
        console.error("Usage: project-integration.ts route <project> <message> [--file=<type>]");
        process.exit(1);
      }
