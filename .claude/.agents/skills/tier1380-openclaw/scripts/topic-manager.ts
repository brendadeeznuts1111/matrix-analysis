...
const CONFIG_PATH = join(homedir(), ".kimi", "skills", "tier1380-openclaw", "config", "telegram-topics.yaml");
...
class TopicManager {
  ...
  listTopics(): void {
    ...
    for (const [id, topic] of Object.entries(this.config.topics)) {
      ...
      console.log(`  ${topic.icon} ${COLORS.bold}${topic.name}${COLORS.reset} (ID: ${id})${defaultMarker}`);
      console.log(`     ${COLORS.gray}${topic.description}${COLORS.reset}`);
      console.log(`     Priority: ${topic.priority} | Commands: ${topic.allow_commands ? "✓" : "✗"} | Files: ${topic.allow_files ? "✓" : "✗"}`);
      console.log();
    }
  }
  ...
  listSuperTopics(): void {
    ...
    for (const [key, superTopic] of Object.entries(this.config.super_topics)) {
      console.log(`  ${superTopic.icon} ${COLORS.bold}${superTopic.name}${COLORS.reset} (${key})`);
  ...
  showRoutingRules(): void {
    ...
    for (const rule of this.config.routing.content_rules) {
      console.log(`  ${COLORS.cyan}${rule.pattern}${COLORS.reset} → Topic ${rule.topic} (${rule.priority})`);
    }
  ...
  routeMessage(message: string): { topic: number; reason: string } | null {
    ...
    if (cmdMatch) {
      ...
      if (this.config.routing.command_routing[cmd]) {
        return { topic: this.config.routing.command_routing[cmd], reason: `Command: /${cmd}` };
      }
    }
  ...
  testRouting(message: string): void {
    ...
    if (result) {
      ...
      if (topic) {
        console.log(`${COLORS.bold}Topic Name:${COLORS.reset} ${topic.icon} ${topic.name}`);
      }
    }
  }
}
