...
async function showRules(projectName?: string) {
  ...
  if (projectName) {
    ...
    console.log("Project".padEnd(25) + Object.keys(RULE_DESCRIPTIONS).map(r => r.replace("on_", "").substring(0, 4).toUpperCase()).join(" "));
    console.log("-".repeat(80));
    
    for (const [name, project] of Object.entries(config.projects)) {
      const rules = getProjectRules(project);
      const indicators = Object.keys(RULE_DESCRIPTIONS).map(rule => {
...
async function testNotification(projectName: string, eventType: string) {
  ...
  console.log(`   Default Topic: ${project.default_topic} (${topicNames[project.default_topic]})`);
...
switch (command) {
  ...
  case "test":
    if (args[0] && args[1]) {
      ...
      console.log("Events: commit, push, merge, pr, issue, release, file_change, test_failure, deploy");
    }
    break;
