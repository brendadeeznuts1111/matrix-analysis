...
async function routeCommit(projectName: string, hash: string, message: string, author: string) {
...
async function handleMerge(projectName: string, branch: string, hash: string, author: string) {
  const config = await loadConfig();
  const project = config.projects[projectName];
