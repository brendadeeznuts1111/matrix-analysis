...
import { getRGBA, getHexColor, getContrastRatio } from "./lib/color.ts";
import { getMemoryUsage, createPerformanceSnapshot } from "./lib/jsc-monitor.ts";
...
async function runTests() {
  ...
  test("telegram-topics.yaml is valid YAML", async () => {
    const content = await readTextFile(`${import.meta.dir}/../config/telegram-topics.yaml`);
  ...
  test("project-topics.yaml has 3 projects", async () => {
    const content = await readTextFile(`${import.meta.dir}/../config/project-topics.yaml`);
    if (!content) return false;
    try {
      const parsed = parse(content);
  ...
  test("readTextFile reads existing files", async () => {
    const result = await readTextFile(`${import.meta.dir}/../config/telegram-topics.yaml`);
  ...
  test("getRGBA extracts channels", () => {
    ...
    return rgba !== null && rgba.r === 76 && rgba.g === 175 && rgba.b === 80 && rgba.a === 1;
  });
  ...
  test("Git hooks installed for nolarose-mcp-config", async () => {
    const hook1 = await Bun.file(`${process.env.HOME}/.git/hooks/post-commit-topic`).exists();
    const hook2 = await Bun.file(`${process.env.HOME}/.git/hooks/post-merge-topic`).exists();
    return hook1 && hook2;
  });

  test("Git hooks installed for openclaw", async () => {
    const hook1 = await Bun.file(`${process.env.HOME}/openclaw/.git/hooks/post-commit-topic`).exists();
    const hook2 = await Bun.file(`${process.env.HOME}/openclaw/.git/hooks/post-merge-topic`).exists();
    return hook1 && hook2;
  });

  test("Git hooks installed for matrix-agent", async () => {
    const hook1 = await Bun.file(`${process.env.HOME}/matrix-agent/.git/hooks/post-commit-topic`).exists();
    const hook2 = await Bun.file(`${process.env.HOME}/matrix-agent/.git/hooks/post-merge-topic`).exists();
    return hook1 && hook2;
  });
  ...
  test("topic-manager.ts executes", async () => {
    const result = await $`bun ${import.meta.dir}/topic-manager.ts list`.quiet().nothrow();
    return result.exitCode === 0;
  });

  test("project-integration.ts executes", async () => {
    const result = await $`bun ${import.meta.dir}/project-integration.ts list`.quiet().nothrow();
    return result.exitCode === 0;
  });

  test("integration-status.ts executes", async () => {
    const result = await $`bun ${import.meta.dir}/integration-status.ts`.quiet().nothrow();
    return result.exitCode === 0;
  });

  test("notification-manager.ts executes", async () => {
    const result = await $`bun ${import.meta.dir}/notification-manager.ts rules`.quiet().nothrow();
    return result.exitCode === 0;
  });

  test("topic-git-hooks.ts executes", async () => {
    const result = await $`bun ${import.meta.dir}/topic-git-hooks.ts list`.quiet().nothrow();
    return result.exitCode === 0;
  });

  test("project-watch.ts executes", async () => {
    const result = await $`bun ${import.meta.dir}/project-watch.ts status`.quiet().nothrow();
    return result.exitCode === 0;
  });
  ...
  console.log("\n" + "=".repeat(60));
  console.log(`${COLORS.bold}Summary:${COLORS.reset} ${passed} passed, ${failed} failed, ${results.length} total`);
