...
export type ColorInput = string | number | { r: number; g: number; b: number; a?: number } | [number, number, number, number?];
 ...
 */
export function convertColor(input: ColorInput, format: ColorFormat): string | number | RGBA | RGB | number[] | null {
  // @ts-ignore - Bun.color is available at runtime
 ...
 */
export function getRGBAArray(input: ColorInput): [number, number, number, number] | null {
  const result = convertColor(input, "[rgba]");
 ...
 */
export function getANSIColor(input: ColorInput, variant: "ansi" | "ansi-16" | "ansi-256" | "ansi-16m" = "ansi"): string | null {
 ...
 */
export function blendColors(color1: ColorInput, color2: ColorInput, ratio = 0.5): RGBA | null {
 ...
 */
export function meetsWCAG(color1: ColorInput, color2: ColorInput, level: "AA" | "AAA" = "AA"): boolean | null {
 ...
 */
function fallbackConvert(input: ColorInput, format: ColorFormat): string | number | RGBA | RGB | number[] | null {
  // Parse input to RGBA
  ...
  switch (command) {
    ...
    case "topics": {
      ...
      for (const [name, color] of Object.entries(topicColors)) {
        ...
        const reset = "\x1b[0m";
        console.log(`  ${ansi}●${reset} ${name.padEnd(15)} ${info.hex} RGB(${info.rgba?.r},${info.rgba?.g},${info.rgba?.b}) ${info.isLight ? "(light)" : "(dark)"}`);
      }
      break;
    }
