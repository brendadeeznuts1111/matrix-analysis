...
class GitHubWebhookBridge {
  ...
  simulate(event: GitHubEvent, repo: string, action?: string): void {
    ...
    console.log(`${COLORS.bold}🔄 Simulating:${COLORS.reset} ${event}${action ? `:${action}` : ""}`);
    ...
    if (result) {
      ...
      console.log(`${COLORS.bold}Route:${COLORS.reset} ${topicNames[result.topic] || `Topic ${result.topic}`}`);
  ...
  async startServer(port: number = 3000): Promise<void> {
    console.log(`${COLORS.green}🚀 Starting webhook bridge server on port ${port}${COLORS.reset}`);
    console.log(`${COLORS.gray}Configure GitHub webhook to POST to http://localhost:${port}/webhook${COLORS.reset}\n`);
      ...
      async fetch(request) {
        ...
        if (url.pathname === "/webhook" && request.method === "POST") {
          ...
          if (result) {
            ...
            console.log(`  ${COLORS.green}✓${COLORS.reset} Routed to Topic ${result.topic}`);
...
async function main() {
  ...
  switch (command) {
    case "simulate":
      ...
      if (!event || !repo) {
        console.error("Usage: github-webhook-bridge.ts simulate <event> <repo> [action]");
    ...
    default:
      ...
      console.log("  github-webhook-bridge.ts simulate pull_request github.com/user/repo opened");
