...
async function checkPrerequisites(): Promise<boolean> {
  ...
  if (major < 1 || (major === 1 && minor < 3)) {
    console.log(`${COLORS.red}❌ Bun ${bunVersion} is too old. Need 1.3.0+${COLORS.reset}`);
    return false;
  }
...
async function verifySetup(): Promise<void> {
  ...
  for (const { name, cmd } of commands) {
    try {
      const result = await $`bun ${import.meta.dir}/../kimi-shell/kimi-cli.ts ${cmd.split(" ")}`.quiet();
      if (result.exitCode === 0) {
        console.log(`${COLORS.green}✅${COLORS.reset} ${name}`);
      } else {
        console.log(`${COLORS.yellow}⚠️${COLORS.reset} ${name} (exit ${result.exitCode})`);
      }
    } catch {
...
async function main() {
  ...
  if (projects.length === 0) {
    ...
    console.log(`Add projects to ${COLORS.gray}~/.kimi/skills/tier1380-openclaw/config/project-topics.yaml${COLORS.reset}`);
  }
