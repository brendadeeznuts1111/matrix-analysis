...
class ChannelMonitor {
  ...
  displayDashboard(): void {
    ...
    console.log(`  Last Activity: ${stats.lastActivity > 0 
    ...
    for (const msg of recent) {
      ...
      console.log(`  ${COLORS.gray}[${time}]${COLORS.reset} ${topicName} ${priorityColor}[${msg.priority}]${COLORS.reset}`);
      console.log(`    ${msg.content.slice(0, 60)}${msg.content.length > 60 ? "..." : ""}`);
    }
  }
}
...
async function main() {
  ...
  monitor.receiveMessage({ topicId: 1, content: "System status: OK", priority: "normal" });
  monitor.receiveMessage({ topicId: 2, content: "High CPU usage detected", priority: "high" });
  monitor.receiveMessage({ topicId: 5, content: "Log rotation completed", priority: "low" });
  monitor.receiveMessage({ topicId: 7, content: "Deployed new feature", priority: "normal" });
  ...
  switch (command) {
    ...
    case "watch": {
      ...
      const interval = setInterval(() => {
        ...
        const randomPriority = priorities[Math.floor(Math.random() * priorities.length)] as any;
