...
async function handleToolCall(name: string, args: any): Promise<object> {
  switch (name) {
    ...
    case "r2_upload": {
      ...
      const result = await $`bun tools/tier1380-registry.ts r2:upload ${localPath} ${r2Key || ""}`.nothrow();
      return {
    ...
    case "r2_download": {
      ...
      const result = await $`bun tools/tier1380-registry.ts r2:download ${r2Key} ${localPath || ""}`.nothrow();
      return {
        success: result.exitCode === 0,
        output: result.stdout.toString(),
        errors: result.stderr.toString()
    ...
    case "matrix_bridge_proxy": {
      ...
      const bridgePath = `${process.env.HOME}/nolarose-mcp-config/matrix-agent/integrations/openclaw-bridge.ts`;
      const result = await $`bun ${bridgePath} ${target === "matrix" ? "matrix" : "proxy"} ${command} ${args}`.nothrow();
      return {
        success: result.exitCode === 0,
        output: result.stdout.toString(),
        errors: result.stderr.toString()
