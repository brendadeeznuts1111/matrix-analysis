    ...
    case 'validate': {
      ...
      try {
        ...
        if (error instanceof SecurityError) {
          console.error(`${COLORS.red}✗ Security violation: ${error.message}${COLORS.reset}`);
    ...
    case 'test': {
      ...
      for (const test of testCases) {
        try {
          ...
          const status = test.shouldPass ? `${COLORS.green}✓${COLORS.reset}` : `${COLORS.yellow}⚠${COLORS.reset}`;
          console.log(`${status} ${test.cmd} - ${test.shouldPass ? 'Allowed' : 'Should have been blocked'}`);
        } catch (error) {
          const status = !test.shouldPass ? `${COLORS.green}✓${COLORS.reset}` : `${COLORS.red}✗${COLORS.reset}`;
          console.log(`${status} ${test.cmd} - ${!test.shouldPass ? 'Blocked (correct)' : 'Blocked (incorrect)'}`);
        }
      }
      break;
    }
