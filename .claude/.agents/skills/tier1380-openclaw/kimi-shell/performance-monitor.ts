...
class PerformanceMonitor {
  ...
  private render(metrics: SystemMetrics): void {
    ...
    const memColor = metrics.memory.percent > 80 ? COLORS.red : 
                     metrics.memory.percent > 60 ? COLORS.yellow : COLORS.green;
    
    console.log(`\n${COLORS.bold}Memory Usage:${COLORS.reset}`);
    console.log(`  Heap Used:  ${memColor}${this.formatBytes(metrics.memory.used)}${COLORS.reset}`);
    console.log(`  Heap Total: ${COLORS.gray}${this.formatBytes(metrics.memory.total)}${COLORS.reset}`);
    ...
    console.log(`  Uptime:  ${COLORS.gray}${this.formatUptime(metrics.bun.uptime)}${COLORS.reset}`);
  ...
  exportReport(): string {
    const avgMemory = this.history.reduce((a, m) => a + m.memory.percent, 0) / this.history.length;
