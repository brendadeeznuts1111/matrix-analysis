...
const UPDATE_CHECK_URL = 'https://api.github.com/repos/brendadeeznuts1111/matrix-analysis/releases/latest';
...
class AutoUpdateManager {
  ...
  async check(): Promise<ReleaseInfo | null> {
    ...
    try {
      ...
      if (!response.ok) {
        console.error(`${COLORS.red}Failed to check for updates: ${response.status}${COLORS.reset}`);
        return null;
      }
      ...
      console.log(`${COLORS.green}✓${COLORS.reset} You are running the latest version (${CURRENT_VERSION})`);
      return null;
    } catch (error) {
  ...
  async setAutoUpdate(enabled: boolean): Promise<void> {
    ...
    await this.saveState();
    console.log(`${COLORS.green}✓${COLORS.reset} Auto-update ${enabled ? 'enabled' : 'disabled'}`);
  }
  ...
  async checkIfDue(): Promise<void> {
    ...
    if (hoursSinceLastCheck >= CHECK_INTERVAL_HOURS) {
      ...
      if (update && this.state.autoUpdate && !this.state.skippedVersions.includes(update.version)) {
...
async function main() {
  ...
  switch (command) {
    ...
    case 'status': {
      ...
      console.log(`Update available: ${info.updateAvailable ? `${COLORS.green}Yes${COLORS.reset}` : 'No'}`);
