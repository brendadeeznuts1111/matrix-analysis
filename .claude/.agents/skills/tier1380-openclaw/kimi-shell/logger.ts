  ...
  switch (command) {
    ...
    case 'query': {
      ...
      for (const entry of entries) {
        ...
        console.log(`${timestamp} ${levelColor[entry.level]}[${entry.level.toUpperCase()}]${reset} ${entry.message}`);

        if (entry.context && Object.keys(entry.context).length > 0) {
          console.log(`  ${'\x1b[90m'}Context: ${JSON.stringify(entry.context)}${reset}`);
        }
    ...
    default: {
      ...
      console.log('  logger.ts query [level] [n] Query logs (level: debug|info|warn|error|audit)');
