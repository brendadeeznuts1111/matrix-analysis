...
class NotificationManager {
   ...
   */
  async send(options: NotificationOptions, channels?: ('desktop' | 'webhook' | 'console')[]): Promise<void> {
   ...
   */
  async trigger(event: string, message: string, context?: Record<string, unknown>): Promise<void> {
  ...
  switch (command) {
    ...
    case 'rules': {
      ...
      for (const rule of rules) {
        const status = rule.enabled ? `${COLORS.green}●${COLORS.reset}` : `${COLORS.gray}○${COLORS.reset}`;
