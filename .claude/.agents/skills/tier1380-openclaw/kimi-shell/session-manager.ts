...
class SessionManager {
  ...
  async listSessions(): Promise<Session[]> {
    ...
    try {
      const files = await Bun.file(SESSIONS_DIR).stream().catch(() => null);
      if (!files) return [];
  ...
  recordCommand(command: string, result: {
    duration: number;
    exitCode: number;
    output?: string;
  }): void {
...
async function main() {
  ...
  switch (command) {
    case 'create': {
      ...
      console.log(`${COLORS.green}✓${COLORS.reset} Created session: ${COLORS.cyan}${session.name}${COLORS.reset} (${session.id.slice(0, 8)})`);
      break;
    }
    ...
    case 'list': {
      ...
      for (const session of sessions) {
        ...
        const indicator = isCurrent ? `${COLORS.green}●${COLORS.reset}` : 
                         session.state === 'active' ? `${COLORS.yellow}○${COLORS.reset}` : 
                         `${COLORS.gray}○${COLORS.reset}`;
        
        const stateColor = session.state === 'active' ? COLORS.green :
                          session.state === 'suspended' ? COLORS.yellow : COLORS.gray;
        
        console.log(`  ${indicator} ${COLORS.bold}${session.name}${COLORS.reset} ${COLORS.gray}(${session.id.slice(0, 8)})${COLORS.reset}`);
    ...
    case 'switch': {
      ...
      if (success) {
        console.log(`${COLORS.green}✓${COLORS.reset} Switched to session: ${COLORS.cyan}${id.slice(0, 8)}${COLORS.reset}`);
      } else {
    ...
    case 'import': {
      ...
      console.log(`${COLORS.green}✓${COLORS.reset} Imported session: ${COLORS.cyan}${session.name}${COLORS.reset} (${session.id.slice(0, 8)})`);
      break;
    }
    ...
    case 'archive': {
      ...
      if (success) {
        console.log(`${COLORS.green}✓${COLORS.reset} Archived session: ${COLORS.cyan}${id.slice(0, 8)}${COLORS.reset}`);
      } else {
    ...
    case 'delete': {
      ...
      if (success) {
        console.log(`${COLORS.green}✓${COLORS.reset} Deleted session: ${COLORS.cyan}${id.slice(0, 8)}${COLORS.reset}`);
      } else {
        console.error(`${COLORS.red}✗${COLORS.reset} Session not found`);
        process.exit(1);
      }
    ...
    default: {
      ...
      console.log("Usage:");
      console.log("  session create <name> [--profile=<p>] [--desc=<d>]  Create new session");
      console.log("  session list                                         List all sessions");
      console.log("  session switch <id>                                  Switch to session");
      console.log("  session current                                      Show current session");
      console.log("  session export <id> > bundle.json                    Export session");
      console.log("  session import <bundle.json>                         Import session");
      console.log("  session archive <id>                                 Archive session");
      console.log("  session delete <id>                                  Delete session");
      console.log("\nExamples:");
      console.log("  session create prod-work --profile=production --desc='Production work session'");
