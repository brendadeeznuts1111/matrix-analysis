...
class JobQueue {
  ...
  async submit(command: string, args: string[] = [], options: {
    priority?: number;
    maxRetries?: number;
  } = {}): Promise<Job> {
    const id = randomUUID();
...
async function main() {
  ...
  switch (command) {
    case 'run': {
      ...
      console.log(`${COLORS.green}✓${COLORS.reset} Job submitted: ${COLORS.cyan}${job.id.slice(0, 8)}${COLORS.reset}`);
    ...
    case 'list': {
      ...
      for (const job of jobs) {
        ...
        const indicator = job.status === 'running' ? '▶' : 
                         job.status === 'completed' ? '✓' :
                         job.status === 'failed' ? '✗' : '○';

        console.log(`  ${statusColor}${indicator}${COLORS.reset} ${job.command.slice(0, 30).padEnd(30)} ${statusColor}${job.status}${COLORS.reset}`);
        console.log(`    ${COLORS.gray}ID: ${job.id.slice(0, 8)} | Created: ${new Date(job.createdAt).toLocaleTimeString()}${COLORS.reset}`);
        
        if (job.exitCode !== undefined) {
          console.log(`    Exit: ${job.exitCode === 0 ? COLORS.green : COLORS.red}${job.exitCode}${COLORS.reset}`);
        }
        console.log();
      }
      break;
    ...
    case 'cancel': {
      ...
      if (success) {
        console.log(`${COLORS.green}✓${COLORS.reset} Job cancelled: ${COLORS.cyan}${id.slice(0, 8)}${COLORS.reset}`);
      } else {
        console.error(`${COLORS.red}✗${COLORS.reset} Could not cancel job (may be running)`);
        process.exit(1);
      }
      break;
    }
    ...
    case 'delete': {
      ...
      if (success) {
        console.log(`${COLORS.green}✓${COLORS.reset} Job deleted: ${COLORS.cyan}${id.slice(0, 8)}${COLORS.reset}`);
      } else {
    ...
    default: {
      ...
      console.log("  job list [status]               List jobs (pending|running|completed|failed)");
