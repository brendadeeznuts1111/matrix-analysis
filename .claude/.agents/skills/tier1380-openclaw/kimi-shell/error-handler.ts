...
export type ErrorCategory = 'user' | 'system' | 'network' | 'permission' | 'resource' | 'unknown';
...
export class ErrorHandler {
  ...
  private displayError(error: KimiError): void {
    const color = error.severity === 'critical' ? COLORS.red :
                  error.severity === 'high' ? COLORS.yellow : COLORS.cyan;
    
    console.error(`\n${color}${COLORS.bold}Error [${error.code}]${COLORS.reset}`);
    console.error(`${color}${error.message}${COLORS.reset}`);
    
    if (error.retryable) {
      console.error(`${COLORS.gray}(This error may be temporary - retry with: kimi retry)${COLORS.reset}`);
    }
  ...
  private getHelpForError(code: string): string | null {
    const helpMap: Record<string, string> = {
      [ErrorCodes.PROFILE_NOT_FOUND]: "Create the profile with: kimi profile create <name>",
      [ErrorCodes.PLUGIN_NOT_FOUND]: "Install the plugin with: kimi plugin install <name>",
