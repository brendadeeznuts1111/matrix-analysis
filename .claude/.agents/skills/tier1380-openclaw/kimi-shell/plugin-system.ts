...
export class PluginManager {
  ...
  async executeCommand(pluginName: string, commandName: string, args: string[]): Promise<void> {
  ...
  switch (command) {
    case "list": {
      ...
      for (const plugin of manager.getAllPlugins()) {
        console.log(`  ${COLORS.green}●${COLORS.reset} ${COLORS.bold}${plugin.name}${COLORS.reset} v${plugin.version}`);
    ...
    case "commands": {
      ...
      for (const cmd of commands) {
        console.log(`  ${COLORS.cyan}${cmd.plugin}:${cmd.command}${COLORS.reset} - ${cmd.description}`);
      }
      break;
    }
