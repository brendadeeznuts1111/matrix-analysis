...
class SnapshotCron {
		...
		try {
			...
			for (const tenant of tenants) {
				try {
					...
					console.error(`   ❌ Failed for ${tenant}:`, err instanceof Error ? err.message : err);
				}
			}
	...
	private async getActiveTenants(): Promise<string[]> {
		...
		try {
			...
			const rows = db.query("SELECT DISTINCT domain as tenant FROM commits LIMIT 100").all() as Array<{ tenant: string }>;
			db.close();
			return rows.map((r) => r.tenant).filter(Boolean);
		} catch {
			// Fallback: scan snapshot directory
			try {
				const files = await $`ls ${this.config.snapshotDir}/*.tar.gz 2>/dev/null || echo ""`.text();
	...
	private async createSnapshot(tenant: string): Promise<{ path: string; sha256: string; size: number }> {
		// Create snapshot directory
		await $`mkdir -p ${this.config.snapshotDir}`.quiet();
		
		// Generate snapshot using the archiver
		const { $ } = await import("bun");
		const result = await $`bun ${import.meta.dir}/snapshot-create.ts ${tenant} ${this.config.snapshotDir}`.json();
...
if (import.meta.main) {
	...
	const interval = parseInt(args.find((a) => a.startsWith("--interval="))?.split("=")[1] || "24", 10);
