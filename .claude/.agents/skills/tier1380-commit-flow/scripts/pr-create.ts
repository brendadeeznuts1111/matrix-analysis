...
async function generatePRBody(): Promise<string> {
	...
	const commits = await $`git log origin/${branch}..HEAD --pretty=format:"%s" 2>/dev/null || echo ""`.text();
...
async function suggestPRTitle(): Promise<string[]> {
	...
	if (branchMatch) {
		const [, type, tier, desc] = branchMatch;
...
async function createPR(config: PRConfig): Promise<boolean> {
	...
	if (!hasGh) {
		...
		if (match) {
			console.log(`  https://github.com/${match[1]}/pull/new/${await $`git branch --show-current`.text()}`);
		}
		return false;
	}
	
	const args = ["pr", "create", "--title", config.title, "--body", config.body, "--base", config.base];
