...
async function getCommitsInRange(base: string, head: string): Promise<Array<{ hash: string; message: string }>> {
...
async function validateCommits(base: string, head: string): Promise<CommitValidationResult[]> {
	...
	for (const commit of commits) {
		...
		if (commit.message.startsWith("Merge ")) {
			results.push({ hash: commit.hash, message: commit.message, valid: true, errors: [] });
			continue;
		}
...
if (import.meta.main) {
	...
	if (!base) {
		...
		console.log("  validate-commits.ts ${{ github.event.pull_request.base.sha }} ${{ github.event.pull_request.head.sha }}");
		process.exit(1);
	}
