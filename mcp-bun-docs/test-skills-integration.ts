...
export class SkillsMCPServer {
  async listSecurityTools(): Promise<Array<{ name: string; description: string; securityLevel: string }>> {
    return [
      {
  ...
  async getSecurityReport(): Promise<string> {
    ...
    const report = {
      ...
      criticalFeatures: classifications.filter(c => c.securityLevel === "critical").map(c => c.term),
      recommendations: [
