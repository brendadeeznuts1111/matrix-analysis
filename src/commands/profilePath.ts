...
import { expandPathVars, checkPathExists, PATH_LIKE_VARS } from "../lib/pathResolver";
...
export async function profilePathAdd(
  ...
  if (config[position]!.includes(path)) {
    ...
    console.log(`\x1b[33m⚠ "${path}" already in ${variable} ${position}, skipping\x1b[0m`);
    return;
  }
    ...
    if (config.prepend && config.prepend.length > 0) {
      ...
      for (const dir of config.prepend) {
        if (options.resolved) {
          ...
          console.log(`      ${mark} ${expanded}${expanded !== dir ? ` (from ${dir})` : ""}`);
        } else {
          console.log(`      ${dir}`);
        }
      }
