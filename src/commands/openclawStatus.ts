...
const getStatusIcon = (status: string): string => {
  switch (status) {
    case "healthy": return `${fmt.ok("")}●`;
    case "degraded": return `${fmt.warn("")}●`;
    case "down": return `${fmt.fail("")}●`;
    default: return "○";
  }
};
...
export async function openclawStatus(options: { 
  ...
  const display = async () => {
    ...
    for (const r of results) {
      ...
      const statusColor = r.status === "healthy" ? (m: string) => fmt.ok(m) : 
                         r.status === "degraded" ? (m: string) => fmt.warn(m) : (m: string) => fmt.fail(m);
      ...
      if (r.message && r.status !== "healthy") console.log(`   ${statusColor(r.message)}`);
      console.log();
    }
...
export async function openclawHealth(): Promise<void> {
  ...
  const checks = [
    ...
    { name: "Matrix Agent Config", path: `${process.env.HOME}/.matrix/agent/config.json` },
